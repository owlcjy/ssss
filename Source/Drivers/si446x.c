...
INT8U config_table[] = RADIO_CONFIGURATION_DATA_ARRAY;
...
si446x_reply_GET_MODEM_STATUS_map CG_GET_MODEM_STATUS;
...
u8 RSSI_value;
s32 RSSI_real;
	SI446X_START_RX( 0, 0, PACKET_LENGTH,0,8,8 );//修改过 //vRadio_StartRX（channel）；//先进入Rx状态。
    ...
    SI446X_CMD( Pro2Cmd, 2 );
	delay_ms(2);
    //return radio_comm_GetResp(8, Pro2Cmd);
	SI446X_READ_RESPONSE( Pro2Cmd, 8 );


    CG_GET_MODEM_STATUS.MODEM_PEND   = Pro2Cmd[0];
    CG_GET_MODEM_STATUS.MODEM_STATUS = Pro2Cmd[1];
    CG_GET_MODEM_STATUS.CURR_RSSI    = Pro2Cmd[2];
    CG_GET_MODEM_STATUS.LATCH_RSSI   = Pro2Cmd[3];
    CG_GET_MODEM_STATUS.ANT1_RSSI    = Pro2Cmd[4];
    CG_GET_MODEM_STATUS.ANT2_RSSI    = Pro2Cmd[5];
    CG_GET_MODEM_STATUS.AFC_FREQ_OFFSET =  ((u16)Pro2Cmd[6] << 8) & 0xFF00;
    CG_GET_MODEM_STATUS.AFC_FREQ_OFFSET |= (u16)Pro2Cmd[7] & 0x00FF;
		
	//RSSI_value = CG_GET_MODEM_STATUS.CURR_RSSI;
	RSSI_value = CG_GET_MODEM_STATUS.LATCH_RSSI;
//RF_Input_Level_dBm=（RSSI_value/2）-MODEM_RSSI_COMP-70
//	MODEM_RSSI_COMP = 0x40 = 64d
	RSSI_real=(RSSI_value/2)-MODEM_RSSI_COMP-70;
	return RSSI_real;
...
void si446x_part_info(void)
{
    ...
    //radio_comm_SendCmd(2, Pro2Cmd);
    SI446X_CMD( Pro2Cmd, 1 );
    //return radio_comm_GetResp(8, Pro2Cmd);
	SI446X_READ_RESPONSE( Pro2Cmd, 8 );

    VVV[0]   = Pro2Cmd[0];
    VVV[1] = ((u16)Pro2Cmd[1] << 8) & 0xFF00;
    VVV[2]    |= (u16)Pro2Cmd[2] & 0x00FF;
    VVV[3]   = Pro2Cmd[3];
    VVV[4]    = ((u16)Pro2Cmd[4] << 8) & 0xFF00;
    VVV[5]    |= (u16)Pro2Cmd[5] & 0x00FF;
    VVV[6] =  Pro2Cmd[6];
    VVV[7] = Pro2Cmd[7];
